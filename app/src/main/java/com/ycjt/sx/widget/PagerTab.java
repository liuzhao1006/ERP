...
public class PagerTab extends ViewGroup {
    ...
    private int mIndicatorColor = 0xFF56E1A0;// 指示器颜色
    ...
    private void init() {
        // 把一个值从dip转换成px
        mIndicatorHeight = Utils.dp2px(mActivity, mIndicatorHeight);
        mDividerPadding = Utils.dp2px(mActivity, mDividerPadding);
        mTabPadding = Utils.dp2px(mActivity, mTabPadding);
        mDividerWidth = Utils.dp2px(mActivity, mDividerWidth);
        mTabTextSize = Utils.dp2px(mActivity, mTabTextSize);
        // 创建一个scroller
    ...
    private void addTextTab(final int position, String title) {
        ...
        tab.setTextColor(Utils.getColorStateList(mActivity, mTabTextColorResId));
        tab.setBackgroundDrawable(Utils.getDrawable(mActivity, mTabBackgroundResId));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mTabCount - 1; i++) {// 分割线的个数比tab的个数少一个
            ...
            if (child != null) {
                canvas.drawLine(child.getRight(), mDividerPadding, child.getRight(), mContentHeight - mDividerPadding,
                        mDividerPaint);
            }
        }
