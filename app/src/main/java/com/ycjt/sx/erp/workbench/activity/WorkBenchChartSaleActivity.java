...
/**
 * 销售饼状图
 * http://192.168.103.247:8011/Service1.svc/GetOrderHead/
 ...
 */
public class WorkBenchChartSaleActivity extends BaseActivity implements ISimple, OnChartValueSelectedListener {
    ...
    private void findview() {
        ...
        mList = chartUtils.setTime(s4.getText().toString().trim());
    ...
    private void getHttpData(Button btn1, Button btn2) {
        ...
        persenter.getWorkBenchOrderData(bean);
    }
    ...
    public void getHttpLineData(Button btn1) {
        ...
        persenter.getWorkBenchOrderDataLine(bean);
    }
     ...
     * 饼状图回调失败,返回几个数据,
     * 第一:图例的数据没有,要刷新,
     * 第二:饼状体的数据没有,返回一个值,名称为"无数据,请重新查询......"
     *
     * @param s
     */
    @Override
    ...
    public void failedLine(String s) {
        showToast(s, TOAST_SHORT);
        List<CustemObject> custemObjectList = new ArrayList<>();
        custemObjectList.add(new CustemObject("无"));
        mSpinerPopWindows5.refreshData(custemObjectList, -1);//将数据传递给popupwindow刷新
        mAdapters5.notifyDataSetInvalidated();//再次调用刷新
        List<WorkItemJson> workItemJsons = new ArrayList<>();
        WorkItemJson w = new WorkItemJson();
        w.setName("刘朝");
        w.setYear("2017");
        w.setMonth("08");
        w.setBillTotalAmountCount(0.0);
        w.setID("zhaozhao");
        workItemJsons.add(w);
        ChartLineDataManager mapManager = new ChartLineDataManager(this, workItemJsons);
        Map<String, Map<String, Double>> map = mapManager.setLineChartData(mList, "无", s3.getText().toString());
        lineChartManager.setLineData(map, 0.1, 100);
        tvM.setText("无");//获取总额
    }
